...
// Created: 日 1月  1 10:04:48 2017 (+0800)
// Last-Updated: 日 1月  1 10:05:05 2017 (+0800)
//           By: Hongyi Wu(吴鸿毅)
//     Update #: 1
// URL: http://wuhongyi.cn 

http://www.cnblogs.com/cy568searchx/p/3584322.html
...
IGNBRK
    忽略输入中的 BREAK 状态。
BRKINT
    如果设置了 IGNBRK，将忽略 BREAK。如果没有设置，但是设置了 BRKINT，那么 BREAK 将使得输入和输出队列被刷新，如果终端是一个前台进程组的控制终端，这个进程组中所有进程将收到SIGINT 信号。如果既未设置 IGNBRK 也未设置 BRKINT，BREAK 将视为与 NUL 字符同义，除非设置了 PARMRK，这种情况下它被视为序列 /377 /0 /0。
IGNPAR
    忽略桢错误和奇偶校验错。
PARMRK
